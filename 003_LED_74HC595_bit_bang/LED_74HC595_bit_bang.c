...
#define CLK_Pin 1   // PA.1 --> CLK_Pin      WR
#define LATCH_Pin 0 // PA.0 --> LATCH_Pin    CS
#define DATA_Pin 2  // PA.2 --> DATA_Pin     DATA

const uint8_t SEGMENT_SELECT[] = {0xF7, 0xFB, 0xFD, 0xFE};
uint8_t symbol[4];
int i = 0;
float z = 0.0;
volatile bool variable = false;
...
void TIMER_Init(void) {
  ...
  //500--> это делитель (смотрим функцию uint32_t TIMER_Open(TIMER_T *timer, uint32_t u32Mode, uint32_t u32Freq))
  TIMER_EnableInt(TIMER0);
  TIMER_Open(TIMER1, TIMER_PERIODIC_MODE, 500);
  TIMER_DisableInt(TIMER1);
  TIMER_Open(TIMER2, TIMER_PERIODIC_MODE, 500);
  TIMER_DisableInt(TIMER2);
  TIMER_Open(TIMER3, TIMER_PERIODIC_MODE, 500);
  TIMER_DisableInt(TIMER3);
